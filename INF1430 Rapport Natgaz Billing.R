...
csv_path <- "/home/christian/ProjetsessionTELUQ/Projet Finale TELUQ/DATABASE Project/Gaz/NaturalGas_Billing.csv"
...
# Utiliser les colonnes de date appropriées pour la transposition
natural_gas_data_long <- natural_gas_data %>%
  pivot_longer(cols = c("Basic charge", "Delivery charges", "Storage and transport", "Commodity charges", "Tax", "Clean energy levy", "Carbon tax", "Amount"),
               names_to = "Variable",
               values_to = "Consumption") %>%
  mutate(Date = ymd(`Invoice Date`)) %>%
  arrange(Date)
...
lm_model <- lm(Consumption ~ Date + Variable, data = train_data)
lm_predictions <- predict(lm_model, test_data)
lm_rmse <- sqrt(mean((lm_predictions - test_data$Consumption)^2))

# Modèle de forêt aléatoire
rf_model <- randomForest(Consumption ~ Date + Variable, data = train_data)
rf_predictions <- predict(rf_model, test_data)
rf_rmse <- sqrt(mean((rf_predictions - test_data$Consumption)^2))

# Analyse de corrélation
cor_matrix <- cor(natural_gas_data_long %>% select(Consumption))
