...
rf_model <- randomForest(avg_rate ~ Day + Month + Year, data = train_data)
rf_predictions <- predict(rf_model, test_data)
rf_rmse <- sqrt(mean((rf_predictions - test_data$avg_rate)^2))
...
lm_plot <- ggplot() +
  geom_point(aes(x = test_data$avg_rate, y = lm_predictions), color = 'blue') +
