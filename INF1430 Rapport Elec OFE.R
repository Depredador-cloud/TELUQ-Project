# Charger les bibliothèques nécessaires
library(tidyverse)
library(lubridate)
library(randomForest)
library(caret)
library(ggplot2)
library(readr)

# Chemin du fichier OFE
path_ofe <- "/home/christian/ProjetsessionTELUQ/Projet Finale TELUQ/DATABASE Project/Electricity/Electricity_OFE.csv"

# Vérifier si le fichier existe
if (!file.exists(path_ofe)) {
  stop("Le fichier n'existe pas à l'emplacement spécifié.")
}

# Charger les données OFE
data_ofe <- read_csv(path_ofe)

# Afficher les premières lignes du jeu de données
head(data_ofe)

# Vérifier les noms des colonnes
colnames(data_ofe)

# Utiliser la colonne de date et heure correcte
datetime_col <- "unix_ts"

# Assurez-vous que la colonne de date et heure existe
if (!datetime_col %in% colnames(data_ofe)) {
  stop(paste("La colonne", datetime_col, "n'existe pas dans le fichier CSV."))
}

# Convertir la colonne de date et heure en objet Date-Heure
data_ofe[[datetime_col]] <- as.POSIXct(data_ofe[[datetime_col]], origin = "1970-01-01", tz = "UTC")

# Vérifier les valeurs manquantes
sum(is.na(data_ofe))

# Nettoyage des données : Gérer les valeurs manquantes si nécessaire
data_ofe <- na.omit(data_ofe)

# Analyse exploratoire des données : Statistiques descriptives
summary(data_ofe)

# Visualiser la consommation OFE au fil du temps
ggplot(data_ofe, aes(x = !!sym(datetime_col), y = P)) +  # Remplacez "P" par le nom de la colonne de consommation si différent
  geom_line(color = "blue") +
  labs(title = "Consommation OFE au Fil du Temps", x = "Temps", y = "Consommation OFE")

# Extraction de caractéristiques : Extraire des caractéristiques utiles à partir du timestamp
data_ofe <- data_ofe %>%
  mutate(
    annee = year(!!sym(datetime_col)),
    mois = month(!!sym(datetime_col)),
    jour = day(!!sym(datetime_col)),
    heure = hour(!!sym(datetime_col))
  )

# Afficher les premières lignes après l'extraction des caractéristiques
head(data_ofe)
